...
<!-- Tool Interface -->
    ...
    <div class="bg-white rounded-2xl shadow-xl shadow-gray-200/50 border border-gray-100 p-6 sm:p-10">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-8 mb-8">
            ...
            <div class="relative">
                <label class="text-xs font-black text-primary uppercase tracking-widest mb-2 block">ASCII (Decimal)</label>
                <textarea id="output-ascii" class="w-full h-80 p-6 bg-blue-50/10 border border-blue-100 rounded-2xl text-gray-900 text-sm resize-none outline-none font-mono" readonly placeholder="72 101 108 108 111..."></textarea>
        ...
        <div class="bg-gray-50 p-4 rounded-xl border border-gray-100 text-center">
            <p class="text-xs text-gray-400 font-bold uppercase tracking-widest">ASCII Character Encoding (0-255)</p>
        </div>
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const output = document.getElementById('output-ascii');

    input.addEventListener('input', () => {
        const text = input.value;
        if(!text) { output.value = ''; return; }
        output.value = text.split('').map(c => c.charCodeAt(0)).join(' ');
    });
