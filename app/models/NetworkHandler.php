...
class NetworkHandler extends Model {
    ...
    private function validateUrl($hostOrUrl, $allowPrivate = false) {
        $host = preg_replace('/^https?:\/\//i', '', $hostOrUrl);
        $host = explode('/', $host)[0];
        $host = explode(':', $host)[0];
        ...
        if (!$allowPrivate) {
            ...
            if (filter_var($ip, FILTER_VALIDATE_IP) && !filter_var($ip, FILTER_VALIDATE_IP, $flags)) {
                return false;
            }
        }
    ...
    public function pingTest($data) {
        ...
        if (!$this->validateUrl($host)) {
            return "<div style='color:red;'>Access to internal or reserved IPs is restricted.</div>";
        }
