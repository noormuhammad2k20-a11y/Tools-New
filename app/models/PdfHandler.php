...
class PdfHandler extends Model {
    ...
    private function validatePdfFile($tmp, $name) {
        if (!$tmp || !file_exists($tmp)) return false;
        $finfo = new finfo(FILEINFO_MIME_TYPE);
        $type = $finfo->file($tmp);
        if ($type !== 'application/pdf') return false;
        $ext = strtolower(pathinfo($name, PATHINFO_EXTENSION));
        if ($ext !== 'pdf') return false;
        if (preg_match('/\.(php|phtml|php3|php4|php5|phps)/i', $name)) return false;
        return true;
    }
    ...
    public function merge($data, $files = []) {
        ...
        foreach ($_FILES['files']['tmp_name'] as $i => $tmp) {
            if ($_FILES['files']['error'][$i] === UPLOAD_ERR_OK) {
                if ($this->validatePdfFile($tmp, $_FILES['files']['name'][$i])) {
    ...
    public function split($data, $files = []) {
        if (!isset($_FILES['file']) || $_FILES['file']['error'] !== UPLOAD_ERR_OK) return "<div style='color:red;'>No file uploaded or upload error.</div>";
        if (!$this->validatePdfFile($_FILES['file']['tmp_name'], $_FILES['file']['name'])) {
            return "<div style='color:red;'>Please upload a valid PDF file.</div>";
        }
    ...
    public function rotate($data, $files = []) {
        if (!isset($_FILES['file']) || $_FILES['file']['error'] !== UPLOAD_ERR_OK) return "<div style='color:red;'>No file uploaded.</div>";
        if (!$this->validatePdfFile($_FILES['file']['tmp_name'], $_FILES['file']['name'])) {
            return "<div style='color:red;'>Please upload a valid PDF file.</div>";
        }
        $pdfBase64 = base64_encode(file_get_contents($_FILES['file']['tmp_name']));
    ...
    public function fromJpg($data, $files = []) {
        if (!isset($_FILES['file']) || $_FILES['file']['error'] !== UPLOAD_ERR_OK) return "<div style='color:red;'>No image uploaded.</div>";
        
        $tmp = $_FILES['file']['tmp_name'];
        $finfo = new finfo(FILEINFO_MIME_TYPE);
        $mime = $finfo->file($tmp);
        if (!in_array($mime, ['image/jpeg', 'image/png'])) {
            return "<div style='color:red;'>Please upload a valid JPG or PNG image.</div>";
        }
    ...
    public function toJpg($data, $files = []) {
        if (!isset($_FILES['file']) || $_FILES['file']['error'] !== UPLOAD_ERR_OK) return "<div style='color:red;'>No PDF uploaded.</div>";
        if (!$this->validatePdfFile($_FILES['file']['tmp_name'], $_FILES['file']['name'])) {
            return "<div style='color:red;'>Please upload a valid PDF file.</div>";
        }
        $pdfBase64 = base64_encode(file_get_contents($_FILES['file']['tmp_name']));
